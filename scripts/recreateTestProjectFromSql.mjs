...
const SQL_PATH = "supabase/test_all_answer_types_ai_branch_project.sql";
...
function parseProject(sql) {
  ...
  return Object.fromEntries(projectColumns.map((column, index) => [column, values[index]]));
}
...
function parseQuestionRows(sql) {
  ...
  for (let i = 0; i < valuesText.length; i += 1) {
    ...
    rows.push(Object.fromEntries(questionColumns.map((column, index) => [column, parseValue(rawValues[index])])));
    i = closeIndex;
  }
  return rows;
}
...
async function main() {
  ...
  console.log(`Recreated project ${projectId} with ${questions.length} questions.`);
}
