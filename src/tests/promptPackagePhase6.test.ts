...
test("全テンプレートに可視化メタ情報が定義されている", () => {
  ...
  // Phase I-B: 型別深掘りガイダンス5キーは usedPolicies / allowedPlaceholders とも空（純散文）
  ...
  for (const key of keys) {
    ...
    if (!PHASE7_KEYS.has(key) && !GUIDANCE_KEYS.has(key)) {
...
test("buildVersionDiff: policy とテンプレートの差分をまとめて返す", () => {
  ...
  // Phase 7-B: 管理ツール系4キー、Phase I-B: 型別深掘りガイダンス5キー追加で全26キー
  assert.equal(diff.templateChanges.length, 26);
});
