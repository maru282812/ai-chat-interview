...
test("全テンプレートに可視化メタ情報が定義されている", () => {
  ...
  // Phase I-B の型別深掘りガイダンス5キーと、管理画面AIチャットの静的システムメッセージ
  // （管理者向けの応答で、回答者との会話に適用するポリシー軸を持たない）が該当。
  const PROSE_ONLY_KEYS = new Set([
    "probeGuidanceCommon",
    "probeGuidanceText",
  ...
  // 件数は BASE_PROMPT_TEMPLATES を正とする（キーが増えるたびに定数を直さないため）。
  // 本テストの主眼は「全キーに可視化メタが揃っているか」で、総数そのものではない。
  ...
  for (const key of keys) {
    ...
    if (!PHASE7_KEYS.has(key) && !PROSE_ONLY_KEYS.has(key)) {
      assert.ok(def.usedPolicies.length > 0, `${key}: usedPolicies（利用ポリシー）が空`);
    }
    if (!PROSE_ONLY_KEYS.has(key)) {
...
test("buildVersionDiff: policy とテンプレートの差分をまとめて返す", () => {
  ...
  // 差分は BASE_PROMPT_TEMPLATES の全キーを網羅する。
  // 定数で件数を固定するとキー追加のたびに落ちるので、定義側を正として突き合わせる
  // （「1キーでも差分から漏れたら落ちる」という本来の意図はこちらの方が強く保てる）。
  assert.equal(diff.templateChanges.length, Object.keys(BASE_PROMPT_TEMPLATES).length);
  assert.deepEqual(
    diff.templateChanges.map(c => c.key).sort(),
    Object.keys(BASE_PROMPT_TEMPLATES).sort()
  );
});
