...
test("P7-1: BASE_PROMPT_TEMPLATES に Phase 7-A の7キーが定義されている", async () => {
  ...
  // 総数は定数で固定しない（キー追加のたびに落ちるため）。
  // このテストの主眼は上のループ＝Phase 7-A の7キーが揃っていること。
  assert.ok(
    Object.keys(BASE_PROMPT_TEMPLATES).length >= PHASE7_KEYS.length,
    "BASE_PROMPT_TEMPLATES が Phase7-A の7キーを含む"
  );
});
...
test("P7-7: バリデーション・プレビューサービスが新キーを対象に含む", async () => {
  ...
  // プレビューは BASE_PROMPT_TEMPLATES の全キーを解決する。
  // 件数を定数で固定するとキー追加のたびに落ちるため、定義側を正として突き合わせる
  // （「1キーでもプレビューから漏れたら落ちる」意図はこちらの方が強く保てる）。
  const { BASE_PROMPT_TEMPLATES } = await import("../prompts/basePromptTemplates");
  const templates = resolveEffectiveTemplates({});
  assert.deepEqual(
    templates.map((t) => t.key).sort(),
    Object.keys(BASE_PROMPT_TEMPLATES).sort(),
    "プレビューが BASE_PROMPT_TEMPLATES の全キーを返す"
  );
  assert.ok(
