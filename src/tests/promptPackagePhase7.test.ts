...
test("P7-1: BASE_PROMPT_TEMPLATES に Phase 7-A の7キーが定義されている", async () => {
  ...
  // Phase 7-B で管理ツール系4キーが追加され21キー
  assert.equal(Object.keys(BASE_PROMPT_TEMPLATES).length, 21, "合計21キー（既存10 + Phase7-A 7 + Phase7-B 4）");
});
...
test("P7-7: バリデーション・プレビューサービスが新キーを対象に含む", async () => {
  ...
  // プレビュー: 21キー全件が解決される（Phase 7-B で4キー追加）
  const templates = resolveEffectiveTemplates({});
  assert.equal(templates.length, 21, "プレビューが21キーを返す");
  assert.ok(
