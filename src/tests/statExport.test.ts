...
test("codebook: 共通指標(metric)をmetaから導出しcodebook列に出す（未設定は空欄）", () => {
  const withMetric = makeQuestion({
    question_code: "Q5",
    question_type: "single_choice",
    sort_order: 5,
    question_config: {
      options: [{ value: "5", label: "満足" }],
      meta: { metric_code: "satisfaction", metric_direction: "higher_is_better" }
    }
  });
  const withMetricVar = deriveVariableDefinition(withMetric);
  assert.equal(withMetricVar.metric_code, "satisfaction");
  assert.equal(withMetricVar.metric_direction, "higher_is_better");

  // 未設定設問は空文字
  const plainVar = deriveVariableDefinition(Q_SINGLE);
  assert.equal(plainVar.metric_code, "");
  assert.equal(plainVar.metric_direction, "");

  // codebook 行に列が出る
  const rows = buildCodebookRows([withMetricVar, plainVar]);
  assert.equal(rows[0]!.metric_code, "satisfaction");
  assert.equal(rows[0]!.metric_direction, "higher_is_better");
  assert.equal(rows[1]!.metric_code, "");
  assert.equal(rows[1]!.metric_direction, "");
});
